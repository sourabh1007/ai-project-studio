 ...
 * review popup can highlight each changed test on its own and attach a
 * per-method explanation next to it.
 *
 ...
 *
 * Segmentation from a diff alone can never be perfect (a change deep inside a
 * method with no surrounding signature falls under the hunk's section heading,
 * or the file preamble when git supplies none); the goal is a helpful,
