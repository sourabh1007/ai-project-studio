...
| `agents/` | The **agent platform**: a registry + service that make attachable feature agents (Review Board, New Task) first-class — attachment persistence, prerequisite gating, namespaced routes/config, and per-agent usage roll-up — with no core hand-wiring. See [agents.md](agents.md). | `agent-contract.ts`, `agent-registry.ts`, `agent-service.ts`, `review-board-agent.ts`, `new-task-agent.ts` |
