...
/**
 * Build the focused *evidence* prompt used to self-heal a perspective whose
 * first-pass analysis came back without the required investigation detail
 * (missing summary, rationale or checks). The verdict is already decided — this
 * asks the model to do ONE thing well: document, with concrete file/symbol
 * references, exactly what it inspected for this lens and why the current
 * rating holds. It never re-opens findings, so it is a small, high-compliance
 * request that guarantees the detail panel is always populated.
 */
export function buildPerspectiveEvidencePrompt(input: {
  board: ReviewBoard;
  perspective: ReviewPerspective;
  description: string | null;
  changedPaths: readonly string[];
  ...
  const verdict =
    perspective.findings.length > 0
      ? [
          `The lens was rated status="${perspective.status}", risk="${perspective.risk}"`,
          'with these findings you must justify with evidence:',
          ...perspective.findings.map(
            (f) => `- [${f.severity}] ${f.title}: ${f.detail}`,
          ),
        ].join('\n')
      : [
          `The lens was reviewed and found clean — rated status="${perspective.status}",`,
          `risk="${perspective.risk}". Prove that verdict with evidence; do NOT`,
          'assume "green" without showing exactly what you inspected.',
        ].join('\n');
  return [
    'You are a meticulous staff engineer documenting the investigation behind a',
    'pull-request review verdict through ONE specific lens. The verdict is',
    'already decided — your only job now is to record, with concrete evidence,',
    'exactly what was inspected and why the rating holds. Do not change the',
    'verdict and do not raise new findings.',
    '',
    `## Review lens: ${perspective.name}`,
    `Purpose: ${perspective.why}`,
    '',
    ...
    'Reply with ONLY a fenced ```json code block containing a single object:',
    '{',
    '  "summary": string — REQUIRED: one or two sentences stating exactly what',
    '    you checked to reach this rating. Name the specific files/symbols you',
    '    inspected and what you verified about them. No generic phrasing,',
    '  "rationale": [ {  — REQUIRED, non-empty: an ordered, evidence-backed',
    '      narrative that justifies the rating so a reader never has to assume',
    '      it. For a problem/solution lens use exactly: "Problem", "Solution',
    '      implemented", "Why they align", "Verdict". Every detail MUST',
    '      reference concrete code (file/symbol) from the change.',
    '    "label": the step label,',
    '    "detail": the concrete, code-referenced explanation for this step } ],',
    '  "checks": [ {  — REQUIRED, non-empty: a line-by-line audit trail of what',
    '      you actually inspected. One entry per concrete thing you looked at.',
    '    "item": the specific file/symbol or aspect inspected (path — symbol),',
    '    "finding": what you observed there, in one concrete sentence,',
    '    "status": one of "pass" | "concern" | "na" } ]',
    '}',
    'Both `rationale` and `checks` MUST be non-empty and grounded in the listed',
    'changed files. Do not add prose outside the code block.',
  ].join('\n');
}
