...
describe('changedTestMethodNames', () => {
  it('lists only changed, named methods once each in order', () => {
    const diff = [
      "@@ -1,3 +1,4 @@ describe('suite', () => {",
      " it('untouched', () => {",
      '   expect(1).toBe(1);',
      ' });',
      " it('added a case', () => {",
    ...
    expect(changedTestMethodNames(diff)).toEqual(['added a case']);
  });

  it('is empty when nothing named changed', () => {
    expect(changedTestMethodNames('+import os')).toEqual([]);
  });
});
