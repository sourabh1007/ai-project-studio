...
describe('renderResult', () => {
  it('renders status and observations', () => {
    expect(
      renderResult(scenario({ status: 'fail', observations: 'crashed' })),
    ).toContain('- Status: fail');
    expect(
      renderResult(scenario({ status: 'fail', observations: 'crashed' })),
    ).toContain('- Observations: crashed');
  });

  it('falls back when observations are empty', () => {
    expect(renderResult(scenario())).toContain('(none reported)');
  });
});
